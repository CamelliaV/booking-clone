...
public class DetailController {
    ...
    @GetMapping(value = "/")
    public ResponseEntity<Page<Detail>> list(@RequestParam(required = false) Integer current, @RequestParam(required
            = false) Integer pageSize, @RequestParam(required = false) String city) {
        if (current == null) {
            current = 1;
        }
        if (pageSize == null) {
        ...
        QueryWrapper<Detail> query = new QueryWrapper<>();
        query.like("district", city);
        Page<Detail> aPage = detailService.page(new Page<>(current, pageSize), query);
